...
int main(int argc, char **argv)
{
	...
	bool onlyW = false;
	if(Options.existsAs<bool>("onlyW"))
		onlyW = Options.getParameter<bool>("onlyW");
...
*/
	...
	TFile* exisistingEEFile = new TFile("/afs/cern.ch/user/v/vciriolo/work/public/existingEE.root", "READ");
	TH2F* existingEE = (TH2F*)( exisistingEEFile->Get("endcap") );

	//TFile* MCSystematicFile = new TFile("/afs/cern.ch/user/v/vciriolo/work/public/ResidualForSystematic_EE_MC.root", "READ");
	...
	if( isEB == true )
	{
		...
		if( onlyW )         InitializeDeadTT_EB_onlyW(TT_centre_EB);//in this case also masked TT has to be included
		else                InitializeDeadTT_EB(TT_centre_EB);
	...
	else 
	{
		...
		if( onlyW )         InitializeDeadTTEEM(TT_centre_EEM);//in this case also masked TT has to be included
		else                InitializeDeadTTEEM(TT_centre_EEM);

		if( onlyW )         InitializeDeadTTEEP(TT_centre_EEP);//in this case also masked TT has to be included
		else                InitializeDeadTTEEP(TT_centre_EEP);

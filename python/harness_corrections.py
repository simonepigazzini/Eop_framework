...
def HarnessLimits(harnessname):
    ...
    print s_ietamin, s_ietamax, s_iphimin, s_iphimax
...
#parse arguments
parser = OptionParser()
parser.add_option('--default_plots',  action='store_true',        dest='default_plots',   default=False,      help='make default plots')
...
for dirname in os.listdir(options.inputdir):
    ...
    if dirname.find("IEta")!=-1 and dirname.find("IPhi")!=-1:
        ...
        print str(chain_dict[harnessname].Add(fullpath+"/out_file_*_scalemonitoring.root"))#to be properly modified 
